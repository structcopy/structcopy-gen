...
func TestSliceStructConvertLoopAssignment(t *testing.T) {
	t.Parallel()
	sta := model.SliceStructConvertLoopAssignment{
		LHS:           "foo",
		RHS:           "bar",
		Typ:           "[]*TargetStruct",
		StructConvert: "SourceStructToTargetStruct",
	}

	t.Run("String", func(t *testing.T) {
		expected := `if bar != nil {
foo = make([]*TargetStruct, len(bar))
for i, e := range bar{
foo[i] = SourceStructToTargetStruct(e)
}
}
`
		actual := sta.String()
